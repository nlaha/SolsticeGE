...
namespace CPM_GLM_AABB_NS {
    ...
    AABB::AABB(const glm::vec3& center, glm::float32_t radius)
    {
        setNull();
    ...
    void AABB::extend(const glm::vec3& p, glm::float32_t radius)
    {
        glm::vec3 r(radius);
        if (!isNull())
        {
    ...
    void AABB::extendDisk(const glm::vec3& c, const glm::vec3& n, glm::float32_t r)
    {
    ...
    glm::vec3 AABB::getCenter() const
    {
        if (!isNull())
        {
            ...
            return mMin + (d * glm::float32_t(0.5));
        }
        else
        {
            return glm::vec3(0.0);
    ...
    bool AABB::isSimilarTo(const AABB& b, glm::float32_t diff) const
    {
        ...
        glm::vec3 acceptable_diff = ((getDiagonal() + b.getDiagonal()) / glm::float32_t(2.0)) * diff;
